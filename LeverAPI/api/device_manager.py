...
class DeviceManager:
    ...
    def _fetch_device_value(self, device_id):
        ...
        try:
            # シミュレーションデバイスはHTTPリクエストを行わずローカル値を返す
            if device_id.startswith("sim_") or str(device_info.get("ip", "")).startswith("127.0.0."):
                if device_id in self.device_values:
                    value_data = self.device_values[device_id]
                    transformed = transform_value_for_frontend(device_id, value_data, device_info)
                    # シミュレーションでも最終応答時間を更新
                    self.discovery.devices[device_id]["last_seen"] = current_time
                    self.discovery.devices[device_id]["status"] = "online"
                    self.value_cache.set(device_id, transformed, ttl=0.5)
                    return transformed
                return None
