...
(function(){
  ...
  const DEFAULT_RANGE = { minValue: 0, maxValue: 100, unit: '%' };
  let latest = { 
  ...
  // Map device_id to index (lever1 / lever_001 -> 0, lever2 -> 1, etc.)
  ...
  function normalizeValue(valueData) {
    if (!valueData) return null;
    const { value, raw } = valueData;
    // If value looks already normalized (0-100-ish), clamp and return
    if (typeof value === 'number' && value >= 0 && value <= 120) {
      return Math.max(0, Math.min(100, value));
    }
    // If raw is provided, map 0-1023 to 0-100
    if (typeof raw === 'number') {
      const norm = (raw / 1023) * 100;
      return Math.max(0, Math.min(100, norm));
    }
    // Fallback: clamp value if numeric, else null
  ...
  function updateLatest(index, valueData) {
    const normalized = normalizeValue(valueData);
    if (normalized !== null) {
      latest.values[index] = normalized;
    }
    if (valueData && valueData.name) {
      latest.names[index] = valueData.name;
    }
          ...
          if (data.updates && typeof data.updates === 'object') {
            ...
            Object.keys(data.updates).forEach(deviceId => {
              ...
              if (index >= 0 && index < 6) {
                ...
                const norm = normalizeValue(valueData);
                if (norm !== null) {
                  newValues[index] = norm;
                  if (valueData && valueData.name) {
                    latest.names[index] = valueData.name;
                  }
                  hasChanges = true;
