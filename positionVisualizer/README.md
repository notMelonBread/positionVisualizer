...
│   └── style.css           # スタイルシート
├── src/
│   ├── app/                # エントリーポイント(main/overlay)
│   ├── domain/             # ValueRange, DeviceState などのドメインモデル
│   ├── usecases/           # LiveMonitorService, RecordingService など
│   ├── infra/              # WebSocket クライアントやストレージ
│   └── presentation/       # ViewModel, renderer, bindings
├── tools/
...
- **src/domain/**: ValueRange, DeviceConfig, DeviceState など純粋なモデル
- **src/usecases/**: LiveMonitorService, RecordingService, ReplayService などビジネスロジック
- **src/infra/**: WebSocketBridgeClient, SettingsStorage, OverlayChannel など外部接続
- **src/presentation/**: ViewModel、meterRenderer/iconRenderer、DOMバインディング
- **src/app/main.js**: メインウィンドウ初期化
- **src/app/overlayApp.js**: オーバーレイウィンドウ初期化
